...
save_dir = "{}/{}_{}_finetuned_webqsp_all_ents;all_mention_biencoder_all_avg_true_20_true_true_bert_large_qa_linear_joint0.0_top50cands_final_joint_0/".format(all_save_dir, data, split)
...
for i, example in enumerate(tqdm(examples)):
    ...
    scores_mask = (mention_scores > -3) | (torch.log_softmax(torch.tensor(biencoder_dists[i][cands_mask]), 1)[:,0] + torch.sigmoid(torch.tensor(mention_scores)).log() > -5).numpy()
    # scores_mask = (torch.sigmoid(torch.tensor(mention_scores)) > 0.25).numpy() & (torch.log_softmax(torch.tensor(biencoder_dists[i][cands_mask]), 1)[:,0] + torch.sigmoid(torch.tensor(mention_scores)) > 0).numpy()
    ...
    # _, sorted_idxs = (torch.log_softmax(torch.tensor(biencoder_dists[i][cands_mask]), 1)[:,0] + torch.sigmoid(torch.tensor(mention_scores)))[scores_mask].sort()
    ...
    if "gold_triples" in example:
        ...
        for idx, mb in enumerate(pred_triples):
            ...
            try:
                if mention_masked_utterance[mb[1]] == 1 or mention_masked_utterance[min(mb[2], len(mention_masked_utterance))-1] == 1:
                    continue
            except:
                import pdb
                pdb.set_trace()
...
# save
with open(os.path.join("/checkpoint/belindali/entity_link/data/{}/saved_preds".format(data), "{}.jsonl".format(split)), 'w') as wf:
