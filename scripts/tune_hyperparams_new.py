...
def load_dists(all_save_dir, data, split, model):
...
for data in ["WebQuestions", "triviaqa", "nq"]:
    if data == "nq":
    ...
    for split in splits:
        ...
        ) = load_dists(all_save_dir, data, split, model)
        f1s = []
        new_examples = []
        num_correct=0
        num_predicted=0
        ...
        for i, example in enumerate(tqdm(examples)):
            ...
            if "gold_triples" in example:
                ...
                num_overlap = entity_linking_tp_with_overlap(gold_triples, new_ex['pred_triples'])
        ...
        # save
        with open(os.path.join("/checkpoint/belindali/entity_link/data/{}/saved_preds".format(data), "{}_top{}.jsonl".format(split, str(topk))), 'w') as wf:
...
def get_threshold_mask_and_sort(mention_dists, cand_dists, biencoder_dists, valid_cands_mask, threshold, top_mention_sort=True):
    ...
    if top_mention_sort:
        ...
        sorted_filtered_scores = scores[scores_mask]
